...
try {
    ...
    const { getFirestore, collection, addDoc, query, orderBy, limit, getDocs } = await import('https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js');
    ...
    console.warn('Firebase not available, using local-only mode:', error);

    // Fallback: No global leaderboard
    GlobalLeaderboard = {
        async save() {
            console.log('Firebase not available - score saved locally only');
            return false;
        },
        async getTop() {
            return [];
